...
config.plugins.NFR_Skin = ConfigSubsection()
config.plugins.NFR_Skin.refreshInterval = ConfigNumber(default=10)
config.plugins.NFR_Skin.woeid = ConfigNumber(default = 638242)
config.plugins.NFR_Skin.tempUnit = ConfigSelection(default="Celsius", choices = [
				("Celsius", _("Celsius")),
				("Fahrenheit", _("Fahrenheit"))
				])
				
def Plugins(**kwargs):
	return [PluginDescriptor(name=_("NFR_Skin  Configtool"), description=_("Personalize your NFR_Skin  (Skin by stein17)"), where = [PluginDescriptor.WHERE_PLUGINMENU],
	icon="plugin.png", fnc=main)]

def main(session, **kwargs):
	if config.skin.primary_skin.value == "NFR_Skin/skin.xml":
		session.open(NFR_Skin_Config)
	else:
		AddPopup(_('Please activate NFR_Skin  Skin before run the Config Plugin'), type=MessageBox.TYPE_ERROR, timeout=10)
		return []
...
class NFR_Skin_Config(Screen, ConfigListScreen):

	skin = """
		<screen name="NFR_Skin_Config" position="center,center" size="1280,720" title="NFR_Skin Setup" >
			<widget source="Title" render="Label" position="70,47" size="950,43" font="Regular;35" transparent="1" />
	...
	def keyYellow(self):
		if self.myAtileHD_active.value:
			self.session.openWithCallback(self.NFR_SkinScreenCB, NFR_SkinScreens)
		else:
	...
	def select_weather_id_callback(self, res):
		if res and isInteger(res):
			print(res)
			config.plugins.NFR_Skin.woeid.value = int(res)
	...
	def about(self):
		self.session.open(NFR_Skin_About)
...
class NFR_SkinScreens(Screen):

	skin = """
		<screen name="NFR_SkinScreens" position="center,center" size="1280,720" title="NFR_Skin Setup">
			<widget source="Title" render="Label" position="70,47" size="950,43" font="Regular;35" transparent="1" />
